...
export default function Leaders() {
  ...
  const [company, setCompany] = useState("Sunbelt Rentals");
  ...
  const coachingOptions = [
    {
      ...
      title: "Emotional Intelligence & Humility",
      icon: "🧠",
    ...
    },
    {
      ...
      title: "Courage & Personal Accountability",
      icon: "🛡️",
  ...
  // Auto-scroll to form when coaching selected
  const formRef = useRef(null);
  function handleCoachingSelect(option) {
    setSelectedCoaching(option.key);
  ...
  async function onSubmit(e) {
    ...
    if (!selectedCoaching || !selectedAgentId) {
      showStatus("Please select a coaching area first.", "error");
      return;
    }
    if (String(selectedAgentId).includes("placeholder")) {
      const option = coachingOptions.find((c) => c.key === selectedCoaching);
      showStatus(
        `${option?.title || "This"} coaching is coming soon! Please try Emotional Intelligence for now.`,
        "error"
      );
      return;
    }
    ...
    const payload = {
      ...
      coachingArea: selectedCoaching,
      agentId: selectedAgentId,
      scenario: "sunbelt-leadership",
    };
  ...
  async function endSession() {
    try {
      ...
      setIsInCall(false);
      setIsStarting(false);
      showStatus("Coaching session completed!", "success");
    }
  }
  ...
  return (
    <>
      <Head>
        <title>Leadership Development Coaching - Sunbelt Rentals</title>
      ...
      <div className="container">
        <header className="header">
          ...
          <h1 className="main-title">Leadership Development Coaching</h1>
          <h2 className="company-title">Sunbelt Rentals</h2>
          <p className="subtitle">
            You&apos;ve completed your leadership behaviour self-assessment. Choose one area for focused coaching to accelerate your development as a Sunbelt leader.
          </p>
        </header>

        <main>
          {/* Coaching Options Grid */}
          <section className="personas-grid" id="coachingGrid">
            {coachingOptions.map((option) => {
              const selected = selectedCoaching === option.key;
              return (
                <div
                  key={option.key}
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => handleCoachingSelect(option)}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") handleCoachingSelect(option);
                  }}
                >
                  <div className="card-glow" />
                  <div className={`persona-icon ${option.iconClass}`}>{option.icon}</div>
                  <h3 className="persona-title">{option.title}</h3>
                  <p className="persona-description">{option.desc}</p>
          ...
          <section
            ref={formRef}
            className={`form-section ${selectedCoaching ? "visible" : ""}`}
            id="participantForm"
          >
            ...
            <h3 className="form-title">
              {selectedCoaching
                ? `${
                    coachingOptions.find((c) => c.key === selectedCoaching)?.title
                  } Coaching`
                : "Ready to Start Your Coaching"}
            </h3>

            {/* Hide form during call to avoid accidental submit */}
            <form id="coachingForm" onSubmit={onSubmit} style={{ display: isInCall ? "none" : "block" }}>
              ...
              <div className="form-group">
                ...
                <input
                  id="participantEmail"
                  className="form-input"
                  placeholder="your.email@sunbeltrentals.com"
                  value={email}
              ...
              <div className="form-group">
                ...
                <input
                  id="accessCode"
                  className="form-input"
                  placeholder="Enter your coaching access code"
                  value={accessCode}
              ...
              <div className="form-group consent-row">
                <label className="consent-label">
                  ...
                  />
                  <span>
                    I consent to the processing of my data for the purpose of running this coaching session
                    and sending me my development plan. I have read the{" "}
                    <a href="/privacy?from=leaders">Privacy Notice</a>.
                  </span>
                </label>

                <p className="privacy-note">
                  This coaching session is <strong>recorded solely for development purposes</strong> and to generate your
                  personalised development plan. You can request deletion of your data at any time:{" "}
              ...
              <button
                ...
                disabled={!canSubmit}
              >
                {isStarting ? "Connecting…" : "Start Coaching Session"}
          ...
          <footer className="footer">
            <p className="copyright">© 2025 CultureHub Limited. Leadership development for Sunbelt Rentals.</p>
          </footer>
        </main>
      </div>

      {/* Identical CSS to feedback page with coaching-specific adjustments */}
      <style jsx global>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .main-title { font-size: 2.5rem; font-weight: 600; margin-bottom: 10px; color: white; text-shadow: 0 2px 20px rgba(248, 91, 246, 0.3); }
        .company-title { font-size: 1.5rem; font-weight: 500; color: #02f5ec; margin-bottom: 15px; }
        .subtitle { font-size: 1.25rem; color: rgba(255, 255, 255, 0.85); max-width: 700px; margin: 0 auto 50px; line-height: 1.7; font-weight: 300; }

        .personas-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 35px; margin-bottom: 50px; animation: fadeInUp 0.8s ease-out; }
        ...
        .persona-icon {
          width: 80px; height: 80px; border-radius: 50%; margin: 0 auto 25px; display: flex; align-items: center; justify-content: center;
          font-size: 2rem; font-weight: 700; color: white; position: relative; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.3); transition: transform 0.3s ease;
        }
        .persona-card:hover .persona-icon { transform: scale(1.1) rotate(5deg); }

        .coaching-ei { background: linear-gradient(135deg, #02f5ec, #349fef); box-shadow: 0 8px 24px rgba(2, 245, 236, 0.4); }
        .coaching-rm { background: linear-gradient(135deg, #fbbf24, #f59e0b); box-shadow: 0 8px 24px rgba(251, 191, 36, 0.4); }
        .coaching-ca { background: linear-gradient(135deg, #dc2626, #b91c1c); box-shadow: 0 8px 24px rgba(220, 38, 38, 0.4); }

        .persona-title { font-size: 1.4rem; font-weight: 600; margin-bottom: 10px; }
        .persona-description { font-size: 0.95rem; color: rgba(255, 255, 255, 0.75); line-height: 1.7; font-weight: 300; min-height: 80px; }
